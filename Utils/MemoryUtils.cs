...
////////////////////////////////////////////////////////////////////////////
	...
	{
		private static Encoding mLatin = Encoding.GetEncoding(1252);
        private static Encoding mUTF8 = new UTF8Encoding(false, true);

		private static string Base62Space = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
        ...
        public static string FromLatin(byte[] data, int index, int count)
        {
            ...
            return mLatin.GetString(data, index, count);
        }
        ...
        public static string FromUTF(byte[] data, int index, int count)
		{
            ...
            return mUTF8.GetString(data, index, count); 
		}
		...
		public static string ToBase62(byte[] original)
		{
			...
			using (BitStream stream = new BitStream(original)) {        
				...
				while (true)
				{
					...
					if (length == 6)                               
					{
						...
						{
							sb.Append(Base62Space[61]);
							stream.Seek(-1, SeekOrigin.Current);    
						}
						else if ((int)(read[0] >> 3) == 0x1e)       
						{
							sb.Append(Base62Space[60]);
							stream.Seek(-1, SeekOrigin.Current);
						}
						else                                        
						{
							sb.Append(Base62Space[(int)(read[0] >> 2)]);
						}
					}
					else
					{

						sb.Append(Base62Space[(int)(read[0] >> (int)(8 - length))]);
						break;
					}
				}
			}
		...
		public static byte[] FromBase62(string base62)
		{
			...
			using ( BitStream stream = new BitStream(base62.Length * 6 / 8)) {

				foreach (char c in base62)
				{
					// Look up coding table
					int index = Base62Space.IndexOf(c);
		...
		{
            string uDir = System.IO.Path.Combine(System.IO.Path.GetTempPath() , "SGCOMBO");
        ...
        {
            return "http://" + MemoryUtils.TrimSlash(str) + "/";
        }

        public static string AddHttps(string str)
        {
            return "https://" + MemoryUtils.TrimSlash(str) + "/";
        }
		...
		public static bool StructureToByteArray(byte[] bytearray, int offset, object obj)
		{
			...
			{
				bRet = false;
				MemoryUtils.Error("Wrong structure size");
			}
        ...
        public static string DownloadString(string sUrl, string Parameters)
        {
            try
            {
                ...
                byte[] bytes = MemoryUtils.ToLatin(Parameters);
